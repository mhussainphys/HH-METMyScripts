...
HistogramsList2D = [r.TH2D('DeltaR_pt','Phi-Eta;   Phi;   Eta',250,-3.2,3.2,250,0,5) for i in range(len(FilenameList))]
HistogramsList3D = [r.TH3D('DeltaR_pt','pT-Eta-Phi;   pT [GeV];   Eta; Phi',300,0,200,300,0,5, 300, -5,5) for i in range(len(FilenameList))]
#600,-50,50,100,0,0.1
HiggsDecayDict = {
...
def TruthHiggList(event):
    ...
    for i in range(len(event.GenPart_pdgId)):
        ...
        if event.GenPart_pdgId[i] == 25:
            TruthHigg = r.Math.PtEtaPhiMVector()
            TruthHigg.SetCoordinates(event.GenPart_pt[i],event.GenPart_eta[i],event.GenPart_phi[i],event.GenPart_mass[i])
            TruthHiggList.append(TruthHigg)
            NumTruthHiggs = NumTruthHiggs + 1
    return TruthHiggList
    ...
    if TruthPhotonList == [] or RecoPhotonList == []: 
        ...
        for i in range(len(TruthPhotonList)):
            ...
            if len(PossibleCandidates[0]) > 1:
                ...
                TruthPhotonTaggedTo.append(TruthPhotonList[i])
            elif len(PossibleCandidates[0]) == 0:
                UntaggedTruthPhotonList.append(TruthPhotonList[i])
    
    return TaggedPhotonHGGList, TruthPhotonTaggedTo, UntaggedTruthPhotonList
...
def diHiggsPtHistogram(TreeList):
    for tree in TreeList:
        eventCounter = 0        
        for event in tree:
            TruthPhotonHGGList = TruthPhotonList(event)
            TruthBHBBList = TruthBJets(event)  
            if len(TruthPhotonHGGList) == 2 and len(TruthBHBBList) == 2:
                HiggsCandidatePhotons = TruthPhotonHGGList[0] + TruthPhotonHGGList[1]
                HiggsCandidateB = TruthBHBBList[0] + TruthBHBBList[1]
                diHiggs = HiggsCandidatePhotons + HiggsCandidateB
                HistogramsList[TreeList.index(tree)].Fill(diHiggs.Pt())
...
def TransInvMassHistogram(TreeList):
    for tree in TreeList:
        eventCounter = 0        
        for event in tree:
            TruthPhotonHGGList = TruthPhotonList(event)
            ...
            if len(TruthPhotonHGGList) == 2 and len(TruthBHBBList) == 2:
                ...
                diHiggs = HiggsCandidatePhotons + HiggsCandidateB
                RecoJet = r.Math.PtEtaPhiMVector()
                RecoJet.SetCoordinates(diHiggs.Pt(),0,diHiggs.Phi(),diHiggs.M())
                RecoJetList.append(RecoJet)
                ...
                for j in range(len(event.GenPart_genPartIdxMother)):
                    if event.GenPart_genPartIdxMother[j] in HiggsEventList:
                        if HiggsEventList.index(event.GenPart_genPartIdxMother[j]) == 0:
                            ...
                            DaughterParticleEventMassList[0].append(event.GenPart_mass[j])
                        elif HiggsEventList.index(event.GenPart_genPartIdxMother[j]) == 1:
                            DaughterParticleEventList[1].append(event.GenPart_pdgId[j])
                            DaughterParticleEventMassList[1].append(event.GenPart_mass[j])
                FirstPairZG = collections.Counter(DaughterParticleEventList[0]) == collections.Counter([23,22]) 
                SecondPairZG = collections.Counter(DaughterParticleEventList[1]) == collections.Counter([23,22])
                if FirstPairZG and SecondPairZG:
                    print DaughterParticleEventList, DaughterParticleEventMassList
                    Counter = Counter + 1
        print Counter
...
def HiggsPeakHistogram(TreeList, DeltaRcut):
    ...
    for tree in TreeList:
        ...
        for event in tree:
            ...
            TruthPhotonHGGList = TruthPhotonList(event)       
            #TaggedPhotonHGGList, TruthPhotonTaggedTo, UntaggedTruthPhotonList = PhotonTaggerv2(TruthPhotonHGGList, RecoPhotonList, DeltaRcut, event)

            if len(TruthPhotonHGGList) == 2:
                HiggsCandidate = TruthPhotonHGGList[0] + TruthPhotonHGGList[1]
                #DeltaR = r.Math.VectorUtil.DeltaR(TruthPhotonHGGList[0],TruthPhotonHGGList[1])
                if HiggsCandidate.M()>=125.4:     
                    HistogramsList[TreeList.index(tree)].Fill(HiggsCandidate.Pt())
...
def HiggsPeakJetHistogram(TreeList, DeltaRcut):
    ...
    for tree in TreeList:
        ...
        for event in tree:
            ...
            if len(TruthBHBBList) == 2:
                HiggsCandidate = TruthBHBBList[0] + TruthBHBBList[1]
                ...
                #HistogramsList[TreeList.index(tree)].Fill(TaggedBHBBList[0].Pt())
                #HistogramsList[TreeList.index(tree)].Fill(TaggedBHBBList[1].Pt())
                if HiggsCandidate.M()>=125:     
                    HistogramsList[TreeList.index(tree)].Fill(HiggsCandidate.Pt())
            eventCounter += 1
...
#HistogramsList = diHiggsPtHistogram(TreeList)
